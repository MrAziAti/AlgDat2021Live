...
public class HeapSort {
    ...
    static void heapSort(int[]values){
        System.out.println("Før heap sort " + Arrays.toString(values));
        heapify(values);
        System.out.println("Etter heap sort " + Arrays.toString(values));
    ...
    static void heapify(int [] values){
        for(int i = 1; i < values.length; i++){
            ...
            while(parent > 0 && values[parent] > values[current]){
                System.out.println("Bytter " + current + " [" + values[current] + "] med " + parent + " [" + values[parent] + "]");
